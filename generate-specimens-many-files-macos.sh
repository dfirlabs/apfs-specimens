#!/bin/bash
#
# Script to generate APFS test files, that contain many files
...
do
	...
	IMAGE_FILE="${SPECIMENS_PATH}/apfs_single_volume_${NUMBER_OF_FILES}_files"

	echo "Creating: case-insensitive APFS; with: ${NUMBER_OF_FILES} files"
	hdiutil create -fs 'APFS' -size ${IMAGE_SIZE} -type UDIF -volname SingleVolume "${IMAGE_FILE}"
	...
	IMAGE_FILE="${SPECIMENS_PATH}/apfs_single_volume_${NUMBER_OF_FILES}_files_case_sensitive"

	echo "Creating: case-sensitive APFS; with: ${NUMBER_OF_FILES} files"
	hdiutil create -fs 'APFS' -size ${IMAGE_SIZE} -type UDIF -volname SingleVolume "${IMAGE_FILE}"
	hdiutil attach "${IMAGE_FILE}.dmg"
